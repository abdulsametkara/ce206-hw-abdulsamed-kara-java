...
public class PlaylistDAO {
    ...
    public Playlist create(Playlist playlist) {
        ...
        try (Connection conn = DatabaseUtil.getConnection()) {
            ...
            try (PreparedStatement pstmt = conn.prepareStatement(insertSql)) {
                ...
                pstmt.setInt(2, playlist.getUser().getId());
                    ...
                    try (Statement stmt = conn.createStatement();
                        ...
                        if (rs.next()) {
                            ...
                            if (playlist.getSongs() != null && !playlist.getSongs().isEmpty()) {
                                addSongsToPlaylist(playlist.getId(), new ArrayList<>(playlist.getSongs()));
                            }
    ...
    private Playlist mapResultSetToPlaylist(ResultSet rs) throws SQLException {
        ...
        // Find user from user id - for now just create a dummy User with only ID
        User user = new User();
        user.setId(userId);
        
        // Create playlist with new constructor
        Playlist playlist = new Playlist(id, name, description, user, createdAt);
