...
public class SongController {
    ...
    public Song addSong(String title, String artist, String album, String genre, int year, int duration, String filePath) {
        ...
        Song song = new Song(title, artist, album, genre, year, duration, filePath, currentUser);
    ...
    protected boolean isValidFile(String filePath) {
        // In demo mode, always return true to allow testing without actual files
        return true;
    ...
    public boolean updateSong(int songId, String title, String artist, String album, String genre, int year) {
        ...
        if (song.getUser().getId() != currentUser.getId()) {
    ...
    public List<Song> searchSongs(String query) {
        ...
        List<Song> userSongs = songs.stream()
                .filter(song -> song.getUser().getId() == currentUser.getId())
    ...
    public boolean addArtist(String artistName) {
        ...
        // Create a new song with just the artist name to ensure artist is stored in the database
        Song dummySong = new Song();
        dummySong.setTitle("Temporary Song - " + System.currentTimeMillis());
        dummySong.setArtist(artistName);
        dummySong.setGenre("Other");
        dummySong.setDuration(0);
        dummySong.setUser(currentUser);
        
        Song createdSong = songDAO.create(dummySong);
        boolean success = (createdSong != null);
        
        if (success) {
            logger.info("Artist added: {}", artistName);
        } else {
            logger.error("Failed to add artist: {}", artistName);
        }
    ...
    public Album addAlbum(String title, String artist, int year, String genre) {
        ...
        // Create a song with the album to ensure it's stored in the database
        Song dummySong = new Song();
        dummySong.setTitle("Temporary Song - " + System.currentTimeMillis());
        dummySong.setArtist(artist);
        dummySong.setAlbum(title);
        dummySong.setGenre(genre);
        dummySong.setYear(year);
        dummySong.setDuration(0);
        dummySong.setUser(currentUser);
        
        Song createdSong = songDAO.create(dummySong);
        if (createdSong != null) {
            album.setId(1); // Mock ID assignment since we're not directly tracking album IDs
            logger.info("Album added: {} by {}", title, artist);
            return album;
        } else {
            logger.error("Failed to add album: {} by {}", title, artist);
            return null;
        }
    }
    ...
    public List<Album> getUserAlbums() {
        ...
        // Kullanıcının şarkılarını al ve albümleri çıkar
        List<Song> userSongs = songDAO.findByUserId(currentUser.getId());
        
        // Benzersiz albüm adlarını ve sanatçıları topla
        ...
        for (Song song : userSongs) {
            if (song.getAlbum() != null && !song.getAlbum().isEmpty()) {
                ...
                if (!albumMap.containsKey(key)) {
                    ...
                    album.setId(albumMap.size() + 1); // Geçici ID ata
        ...
        logger.info("Retrieved {} albums for user {}", albums.size(), currentUser.getUsername());
