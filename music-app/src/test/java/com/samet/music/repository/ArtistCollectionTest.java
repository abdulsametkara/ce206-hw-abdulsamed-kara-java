...
public class ArtistCollectionTest {
    ...
    public void testGetAll() {
        ...
        assertTrue("Artist1 koleksiyonda olmalı", foundArtist1);
        assertTrue("Artist2 koleksiyonda olmalı", foundArtist2);
    }
    ...
    public void testLoadFromDatabase() throws Exception {
        ...
        // Assert
        Artist loaded = artistCollection.getById(artistId);
        assertNotNull("Sanatçı veritabanından yüklenmeli", loaded);
        assertEquals("Yüklenen sanatçı ID'si doğru olmalı", artistId, loaded.getId());
        assertEquals("Yüklenen sanatçı adı doğru olmalı", testArtist.getName(), loaded.getName());
    }
